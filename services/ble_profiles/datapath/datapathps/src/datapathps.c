...
#define DATAPATHPS_SERVICE_UUID_16BIT       0x4100
#define DATAPATHPS_TX_CHAR_VAL_UUID_16BIT   0x4101
#define DATAPATHPS_RX_CHAR_VAL_UUID_16BIT   0x4102
#define DATAPATHPS_STREAM_CHAR_VAL_UUID_16BIT   0x4103
...
const struct attm_desc datapathps_att_db[DATAPATHPS_IDX_NB] =
{
    ...
    // TX Characteristic Value
#ifdef  __APP_WL_SMARTVOICE__
    [DATAPATHPS_IDX_TX_VAL]     =   {DATAPATHPS_TX_CHAR_VAL_UUID_16BIT, PERM(NTF, ENABLE) | PERM(RD, ENABLE), PERM(RI, ENABLE), DATAPATHPS_MAX_LEN},
#else
    ...
    // RX Characteristic Value
#ifdef  __APP_WL_SMARTVOICE__
    [DATAPATHPS_IDX_RX_VAL]     =   {DATAPATHPS_RX_CHAR_VAL_UUID_16BIT, PERM(NTF, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE), PERM(RI, ENABLE), DATAPATHPS_MAX_LEN},
#else
...
#ifdef  __APP_WL_SMARTVOICE__
    // RX Characteristic - Client Characteristic Configuration Descriptor
    [DATAPATHPS_IDX_RX_NTF_CFG] =   {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE), 0, 0},
#endif
	// RX Characteristic - Characteristic User Description Descriptor
...
#ifdef  __APP_WL_SMARTVOICE__
    // TX Characteristic Declaration
    [DATAPATHPS_IDX_STREAM_CHAR]    =   {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
    // TX Characteristic Value
    [DATAPATHPS_IDX_STREAM_VAL]     =   {DATAPATHPS_STREAM_CHAR_VAL_UUID_16BIT, PERM(NTF, ENABLE), PERM(RI, ENABLE) , DATAPATHPS_MAX_LEN},
    // TX Characteristic - Client Characteristic Configuration Descriptor
    [DATAPATHPS_IDX_STREAM_NTF_CFG] =   {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE), 0, 0},
    // TX Characteristic - Characteristic User Description Descriptor
    [DATAPATHPS_IDX_STREAM_DESC]    =   {ATT_DESC_CHAR_USER_DESCRIPTION, PERM(RD, ENABLE), PERM(RI, ENABLE), 32},
#endif
};
