...
{	
    ...
    if (datapathps_env != NULL)
    {
        ...
        else if (param->handle == (datapathps_env->shdl + DATAPATHPS_IDX_RX_NTF_CFG))
        {
            uint16_t value = 0x0000;
            ...
            if (value == PRF_CLI_STOP_NTFIND) {
                datapathps_env->isRxNotificationEnabled[conidx] = false;
            } else if (value == PRF_CLI_START_NTF) {
                datapathps_env->isRxNotificationEnabled[conidx] = true;
            } else {
                status = PRF_APP_ERROR;
            }
            ...
            if (status == GAP_ERR_NO_ERROR) {
                //Inform APP of TX ccc change
                struct ble_datapath_tx_notif_config_t * ind = KE_MSG_ALLOC(DATAPATHPS_RX_CCC_CHANGED,
                        prf_dst_task_get(&datapathps_env->prf_env, conidx),
                        prf_src_task_get(&datapathps_env->prf_env, conidx),
                        ble_datapath_tx_notif_config_t);

                ind->isNotificationEnabled = datapathps_env->isRxNotificationEnabled[conidx];

                ke_msg_send(ind);
            }
        }

        else if (param->handle == (datapathps_env->shdl + DATAPATHPS_IDX_STREAM_NTF_CFG))
        {
            uint16_t value = 0x0000;
            ...
            if (value == PRF_CLI_STOP_NTFIND) {
                datapathps_env->isStreamNotificationEnabled[conidx] = false;
            } else if (value == PRF_CLI_START_NTF) {
                datapathps_env->isStreamNotificationEnabled[conidx] = true;
            } else {
                status = PRF_APP_ERROR;
            }
            ...
            if (status == GAP_ERR_NO_ERROR) {
                //Inform APP of TX ccc change
                struct ble_datapath_tx_notif_config_t * ind = KE_MSG_ALLOC(DATAPATHPS_STREAM_CCC_CHANGED,
                        prf_dst_task_get(&datapathps_env->prf_env, conidx),
                        prf_src_task_get(&datapathps_env->prf_env, conidx),
                        ble_datapath_tx_notif_config_t);

                ind->isNotificationEnabled = datapathps_env->isStreamNotificationEnabled[conidx];
...
#ifdef  __APP_WL_SMARTVOICE__
    } else if (param->handle == (datapathps_env->shdl + DATAPATHPS_IDX_STREAM_DESC)) {
        cfm->length = sizeof(custom_stream_desc);
        memcpy(cfm->value, custom_stream_desc, cfm->length);
#endif
...
static void send_stream_notifiction(uint8_t conidx, const uint8_t* ptrData, uint32_t length)
{
    struct datapathps_env_tag *datapathps_env = PRF_ENV_GET(DATAPATHPS, datapathps);

    if (datapathps_env->isStreamNotificationEnabled[conidx]) {
        // Allocate the GATT notification message
        struct gattc_send_evt_cmd *report_ntf = KE_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
                                                KE_BUILD_ID(TASK_GATTC, conidx), prf_src_task_get(&datapathps_env->prf_env, conidx),
                                                gattc_send_evt_cmd, length);
        ...
        // Fill in the parameter structure
        report_ntf->operation = GATTC_NOTIFY;
        report_ntf->handle    = datapathps_env->shdl + DATAPATHPS_IDX_STREAM_VAL;
        // pack measured value in database
        report_ntf->length    = length;
        memcpy(report_ntf->value, ptrData, length);
        // send notification to peer device
...
__STATIC int send_stream_via_notification_handler(ke_msg_id_t const msgid,
        struct ble_datapath_send_data_req_t const *param,
        ke_task_id_t const dest_id,
        ke_task_id_t const src_id)
{
    send_stream_notifiction(param->connecionIndex, param->value, param->length);
    return (KE_MSG_CONSUMED);
}

static void send_wl_data_notifiction(uint8_t conidx, const uint8_t* ptrData, uint32_t length)
{
    struct datapathps_env_tag *datapathps_env = PRF_ENV_GET(DATAPATHPS, datapathps);

    if (datapathps_env->isRxNotificationEnabled[conidx]) {
        // Allocate the GATT notification message
        struct gattc_send_evt_cmd *report_ntf = KE_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
                                                KE_BUILD_ID(TASK_GATTC, conidx), prf_src_task_get(&datapathps_env->prf_env, conidx),
                                                gattc_send_evt_cmd, length);
        ...
        // Fill in the parameter structure
        report_ntf->operation = GATTC_NOTIFY;
        report_ntf->handle    = datapathps_env->shdl + DATAPATHPS_IDX_RX_VAL;
        // pack measured value in database
        report_ntf->length    = length;
        memcpy(report_ntf->value, ptrData, length);
        // send notification to peer device
...
__STATIC int send_wl_data_via_notification_handler(ke_msg_id_t const msgid,
        struct ble_datapath_send_data_req_t const *param,
        ke_task_id_t const dest_id,
        ke_task_id_t const src_id)
{
    send_wl_data_notifiction(param->connecionIndex, param->value, param->length);
    return (KE_MSG_CONSUMED);
}
...
#ifdef  __APP_WL_SMARTVOICE__
    {DATAPATHPS_SEND_STREAM_VIA_NOTIFICATION,   (ke_msg_func_t) send_stream_via_notification_handler},
    {DATAPATHPS_SEND_WL_DATA_VIA_NOTIFICATION,  (ke_msg_func_t) send_wl_data_via_notification_handler},
#endif
