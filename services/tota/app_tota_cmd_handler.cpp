...
static void app_tota_cmd_handler_remove_waiting_rsp_timeout_supervision(uint16_t entryIndex)
{
    ...
    //TOTA_LOG_DBG("decrease the supervisor timer to %d", tota_cmd_handler_env.timeoutSupervisorCount);
...
static void app_tota_cmd_handler_add_waiting_rsp_timeout_supervision(uint16_t entryIndex)
{
    ...
    //TOTA_LOG_DBG("increase the supervisor timer to %d", tota_cmd_handler_env.timeoutSupervisorCount);
...
{
	TOTA_LOG_DBG("TOTA Receive data:");
	...
	{
		TOTA_LOG_DBG("[%s]error TOTA_INVALID_CMD",__func__);
	...
	{
		TOTA_LOG_DBG("[%s]error TOTA_PARAM_LEN_OUT_OF_RANGE",__func__);
...
{
    //int ret=0;;
	TOTA_LOG_DBG("######Receive data len:%d", dataLength);
