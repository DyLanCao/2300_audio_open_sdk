...
//    kLog10_2 = 49321; // 10*log10(2)  in Q14
    ...
    tmp16no1 += wl_WebRtcSpl_DivW32W16ResW16(tmp32no1 + (kCompRatio >> 1), kCompRatio);
    ...
    zeroGainLvl -= wl_WebRtcSpl_DivW32W16ResW16(tmp32no1 + ((kCompRatio - 1) >> 1),
                                             kCompRatio - 1);
    ...
    diffGain = wl_WebRtcSpl_DivW32W16ResW16(tmp32no1 + (kCompRatio >> 1), kCompRatio);
    ...
    limiterLvlX = analogTarget - limiterOffset;
    limiterIdx = 2
            + wl_WebRtcSpl_DivW32W16ResW16(WEBRTC_SPL_LSHIFT_W32((int32_t)limiterLvlX, 13),
                                        WEBRTC_SPL_RSHIFT_U16(kLog10_2, 1));
    tmp16no1 = wl_WebRtcSpl_DivW32W16ResW16(limiterOffset + (kCompRatio >> 1), kCompRatio);
    ...
    for (i = 0; i < 32; i++)
    {
        ...
        inLevel = wl_WebRtcSpl_DivW32W16(tmp32, kCompRatio); // Q14
        ...
        if (limiterEnable && (i < limiterIdx))
        {
            ...
            y32 = wl_WebRtcSpl_DivW32W16(tmp32 + 10, 20);
        }
        if (y32 > 39000)
        {
                                   ...
                                   int16_t nrSamples) // (i) number of samples
{
    ...
    tmp32 = wl_WebRtcSpl_DivW32W16(tmp32, state->stdLongTerm);
