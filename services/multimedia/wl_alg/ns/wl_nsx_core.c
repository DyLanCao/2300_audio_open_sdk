...
static void UpdateNoiseEstimate(NsxInst_t* inst, int offset) {
  ...
  tmp16 = wl_WebRtcSpl_MaxValueW16(inst->noiseEstLogQuantile + offset,
                                   inst->magnLen);
...
static void SynthesisUpdateC(NsxInst_t* inst,
  ...
  wl_WebRtcSpl_ZerosArrayW16(inst->synthesisBuffer
...
NoiseEstimation wl_WebRtcNsx_NoiseEstimation;
PrepareSpectrum wl_WebRtcNsx_PrepareSpectrum;
SynthesisUpdate wl_WebRtcNsx_SynthesisUpdate;
AnalysisUpdate wl_WebRtcNsx_AnalysisUpdate;

#if (defined WEBRTC_DETECT_ARM_NEON || defined WEBRTC_ARCH_ARM_NEON)
// Initialize function pointers for ARM Neon platform.
static void WebRtcNsx_InitNeon(void) {
  wl_WebRtcNsx_NoiseEstimation = WebRtcNsx_NoiseEstimationNeon;
  wl_WebRtcNsx_PrepareSpectrum = WebRtcNsx_PrepareSpectrumNeon;
  wl_WebRtcNsx_SynthesisUpdate = WebRtcNsx_SynthesisUpdateNeon;
  wl_WebRtcNsx_AnalysisUpdate = WebRtcNsx_AnalysisUpdateNeon;
}
#endif

void wl_WebRtcNsx_CalcParametricNoiseEstimate(NsxInst_t* inst,
...
// Initialize state
int32_t wl_WebRtcNsx_InitCore(NsxInst_t* inst, uint32_t fs) {
  int i;
  ...
  if (inst->real_fft != NULL) {
    wl_WebRtcSpl_FreeRealFFT(inst->real_fft);
  }
  
  inst->real_fft = wl_WebRtcSpl_CreateRealFFT(inst->stages);
  if (inst->real_fft == NULL) {
    return -1;
  }
  ...
  wl_WebRtcSpl_ZerosArrayW16(inst->analysisBuffer, ANAL_BLOCKL_MAX);
  wl_WebRtcSpl_ZerosArrayW16(inst->synthesisBuffer, ANAL_BLOCKL_MAX);

  // for HB processing
  wl_WebRtcSpl_ZerosArrayW16(inst->dataBufHBFX, ANAL_BLOCKL_MAX);
  // for quantile noise estimation
  wl_WebRtcSpl_ZerosArrayW16(inst->noiseEstQuantile, HALF_ANAL_BLOCKL);
  ...
  wl_WebRtcSpl_MemSetW16((int16_t*)inst->noiseSupFilter, 16384, HALF_ANAL_BLOCKL);
  ...
  wl_WebRtcSpl_ZerosArrayW16(inst->histLrt, HIST_PAR_EST);
  wl_WebRtcSpl_ZerosArrayW16(inst->histSpecDiff, HIST_PAR_EST);
  wl_WebRtcSpl_ZerosArrayW16(inst->histSpecFlat, HIST_PAR_EST);
  ...
  //default mode
  //wl_WebRtcNsx_set_policy_core(inst, 0);
  ...
  wl_WebRtcNsx_NoiseEstimation = NoiseEstimationC;
  wl_WebRtcNsx_PrepareSpectrum = PrepareSpectrumC;
  wl_WebRtcNsx_SynthesisUpdate = SynthesisUpdateC;
  wl_WebRtcNsx_AnalysisUpdate = AnalysisUpdateC;
...
int wl_WebRtcNsx_set_policy_core(NsxInst_t* inst, int mode) {
...
void wl_WebRtcNsx_FeatureParameterExtraction(NsxInst_t* inst, int flag) {
  uint32_t tmpU32;
  uint32_t histIndex;
...
void wl_WebRtcNsx_ComputeSpectralFlatness(NsxInst_t* inst, uint16_t* magn) {
  uint32_t tmpU32;
...
void wl_WebRtcNsx_ComputeSpectralDifference(NsxInst_t* inst, uint16_t* magnIn) {
...
void wl_WebRtcNsx_SpeechNoiseProb(NsxInst_t* inst, uint16_t* nonSpeechProbFinal,
  ...
  if (inst->weightSpecFlat) {
    ...
    tmp32no1 = (int32_t)wl_WebRtcSpl_DivU32U16(WEBRTC_SPL_LSHIFT_U32(tmpU32no2,
                                                                        nShifts), 25); //Q14
    tmpU32no1 = wl_WebRtcSpl_DivU32U16(WEBRTC_SPL_LSHIFT_U32(tmpU32no2, nShifts), 25); //Q14
    // compute indicator function: sigmoid map
    // FLOAT code
  ...
  indPriorFX16 = wl_WebRtcSpl_DivW32W16ResW16(98307 - indPriorFX, 6); // Q14
...
void wl_WebRtcNsx_DataAnalysis(NsxInst_t* inst, short* speechFrame, uint16_t* magnU16) {
  ...
  // Update analysis buffer for lower band, and window data before FFT.
  wl_WebRtcNsx_AnalysisUpdate(inst, winData, speechFrame);

  // Get input energy
  inst->energyIn = wl_WebRtcSpl_Energy(winData, (int)inst->anaLen, &(inst->scaleEnergyIn));

  // Reset zero input flag
  inst->zeroInputSignal = 0;
  // Acquire norm for winData
  maxWinData = wl_WebRtcSpl_MaxAbsValueW16(winData, inst->anaLen);
  ...
  wl_WebRtcSpl_RealForwardFFT(inst->real_fft, realImag, winData);
  ...
  if (inst->blockIndex >= END_STARTUP_SHORT) {
    for (i = 1, j = 2; i < inst->anaLen2; i += 1, j += 2) {
      ...
      magnU16[i] = (uint16_t)wl_WebRtcSpl_SqrtFloor(tmpU32no1); // Q(normData-stages)
    ...
    tmp_2_w32 = wl_WebRtcSpl_DivW32W16(tmp_2_w32, matrix_determinant); // Q11
    ...
    if (tmp_2_w32 > 0) {
      ...
      // flat spectrum.
      tmp_1_w32 = wl_WebRtcSpl_DivW32W16(tmp_2_w32, matrix_determinant); // Q14
      inst->pinkNoiseExp += WEBRTC_SPL_SAT(16384, tmp_1_w32, 0); // Q14
    }
  }
}

void wl_WebRtcNsx_DataSynthesis(NsxInst_t* inst, short* outFrame) {
  int32_t energyOut;
  ...
  if (inst->zeroInputSignal) {
    ...
    WEBRTC_SPL_MEMCPY_W16(inst->synthesisBuffer,
                          inst->synthesisBuffer + inst->blockLen10ms,
                          inst->anaLen - inst->blockLen10ms);
    wl_WebRtcSpl_ZerosArrayW16(inst->synthesisBuffer + inst->anaLen - inst->blockLen10ms,
                            inst->blockLen10ms);
    return;
  }

  // Filter the data in the frequency domain, and create spectrum.
  wl_WebRtcNsx_PrepareSpectrum(inst, realImag);

  // Inverse FFT output will be in rfft_out[].
  outCIFFT = wl_WebRtcSpl_RealInverseFFT(inst->real_fft, realImag, rfft_out);
  ...
  if (inst->gainMap == 1 &&
      ...
      inst->energyIn > 0) {
    energyOut = wl_WebRtcSpl_Energy(inst->real, (int)inst->anaLen, &scaleEnergyOut); // Q(-scaleEnergyOut)
  ...
  wl_WebRtcNsx_SynthesisUpdate(inst, outFrame, gainFactor);
}

int wl_WebRtcNsx_ProcessCore(NsxInst_t* inst, short* speechFrame, short* speechFrameHB,
  ...
  wl_WebRtcNsx_DataAnalysis(inst, speechFrame, magnU16);

  if (inst->zeroInputSignal) {
    wl_WebRtcNsx_DataSynthesis(inst, outFrame);
  ...
  // Compute spectral flatness on input spectrum
  wl_WebRtcNsx_ComputeSpectralFlatness(inst, magnU16);

  // quantile noise estimate
  wl_WebRtcNsx_NoiseEstimation(inst, magnU16, noiseU32, &qNoise);
  ...
  if (inst->blockIndex < END_STARTUP_SHORT) {
    ...
    if (inst->pinkNoiseExp) {
      pink_noise_exp_avg = (int16_t)wl_WebRtcSpl_DivW32W16(inst->pinkNoiseExp,
                                                              (int16_t)(inst->blockIndex + 1)); // Q14
      pink_noise_num_avg = wl_WebRtcSpl_DivW32W16(inst->pinkNoiseNumerator,
                                               (int16_t)(inst->blockIndex + 1)); // Q11
      wl_WebRtcNsx_CalcParametricNoiseEstimate(inst,
                                            pink_noise_exp_avg,
                                            pink_noise_num_avg,
                                            kStartBand,
                                            &noise_estimate,
    ...
    for (i = 0; i < inst->magnLen; i++) {
      ...
      noiseU32[i] = wl_WebRtcSpl_DivU32U16(tmpU32no1 + tmpU32no2, END_STARTUP_SHORT);
  ...
  if (inst->blockIndex < END_STARTUP_LONG) {
    ...
    inst->timeAvgMagnEnergy = wl_WebRtcSpl_DivU32U16(inst->timeAvgMagnEnergyTmp,
                                                  inst->blockIndex + 1);
  }
  ...
  wl_WebRtcNsx_ComputeSpectralDifference(inst, magnU16);
  ...
  //update histogram
  wl_WebRtcNsx_FeatureParameterExtraction(inst, flag);
  ...
  //compute speech/noise probability
  wl_WebRtcNsx_SpeechNoiseProb(inst, nonSpeechProbFinal, priorLocSnr, postLocSnr);
    ...
    if (inst->blockIndex < END_STARTUP_SHORT) {
      ...
      tmpU32no2 = WEBRTC_SPL_UMUL_16_16(noiseSupFilterTmp[i],
                                        (uint16_t)(END_STARTUP_SHORT
                                                         - inst->blockIndex));
      tmpU32no1 += tmpU32no2;
      inst->noiseSupFilter[i] = (uint16_t)wl_WebRtcSpl_DivU32U16(tmpU32no1,
                                                                    END_STARTUP_SHORT);
    }
  }  // end of loop over frequencies
  //done with step3
