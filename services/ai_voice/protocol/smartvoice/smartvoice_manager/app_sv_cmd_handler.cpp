...
uint32_t app_sv_rcv_cmd(void *param1, uint32_t param2)
{
	...
	ai_mailbox_put(SIGN_AI_CMD_COME, param2);
...
uint32_t app_sv_parse_cmd(void *param1, uint32_t param2)
{
    ...
    if(DeCQueue(&ai_stream_buf_queue, ptrData, param2)) {
        TRACE("%s DeCQueue error len %d", __func__, param2);
        return 1;
    }
    
    TRACE("%s data len %d", __func__, param2);
    ...
    while(data_length) {
        ...
        if(data_length < pPayload->paramLen) {
            TRACE("%s error data_length %d paramLen %d", __func__, data_length, pPayload->paramLen);
            return 2;
        }
        TRACE("%s data_length %d  paramLen %d", __func__, data_length, pPayload->paramLen);
        data_length -= pPayload->paramLen;

    	// check command code
    	if (pPayload->cmdCode >= OP_COMMAND_COUNT) {
    		TRACE("SV_INVALID_COMMAND_CODE");
    		return 3;
    	}

    	// check parameter length
    	if (pPayload->paramLen > sizeof(pPayload->param)) {
    	    TRACE("SV COMMAND PARAM ERROR LENGTH");
    		return 4;
    	}
...
static bool app_sv_cmd_send(APP_SV_CMD_PAYLOAD_T* ptPayLoad)
{
    uint8_t ret = 0;
    
    TRACE("Send cmd:");
	TRACE("ptPayLoad: 0x%02x", ptPayLoad->cmdCode);

    ret = ai_cmd_transport((uint8_t *)(ptPayLoad ),
