	...
	uint16_t app_task, uint8_t sec_lvl, void* params)
{
	...
	TRACE("attm_ama_create_db_128 returns %d start handle is %d", status, *start_hdl);

    //-------------------- allocate memory required for the profile  ---------------------
    if (status == ATT_ERR_NO_ERROR)
    {
        // Allocate SMARTVOICE required environment variable
        struct ai_env_tag* ama_env =
...
{
    struct ai_env_tag* ama_env = (struct ai_env_tag*) env->env;
									  ...
									  ke_task_id_t const src_id)
{
	...
	struct ai_env_tag *ama_env = PRF_ENV_GET(AI, ai);
...
{	
	...
	TRACE("gattc_write_req_ind_handler ama_env 0x%x write handle %d shdl %d", 
...
{	
	...
	TRACE("gattc_read_req_ind_handler read handle %d shdl %d", param->handle, ama_env->shdl);
