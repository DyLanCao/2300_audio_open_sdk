...
unsigned long vad_store_buf[2048] = {0};
unsigned long vad_store_buf_len = 0;
...
static int voice_detector_vad_stop(struct voice_detector_dev *pdev)
{
    ...
    //    TRACE("0x%x, 0x%x", i, test_buf[i]);

    memcpy((void*)(vad_store_buf), (void*)(pbuf), len);
    vad_store_buf_len = len;
    hal_codec_vad_stop();
    return 0;
}
...
#endif
//    VD_TRACE("%s, cpu freq=%d", __func__, hal_sys_timer_calc_cpu_freq(5,0));
    return 0;
}
...
{
    VD_TRACE("%s, buf=%x, len=%d", __func__, (uint32_t)buf, len);
    //TODO: print data
}
...
static uint32_t mic_data_come(uint8_t *buf, uint32_t len)
{
    ...
    if (retcode == CSPOTTER_SUCCESS) {
        ...
        TRACE("##### CSpotterGetResult return ID : %d\n", id);

        score = CSpotter_GetResultScore(h_CSpotter);
        TRACE("CSpotter_GetResultScore return Score: %d", score);
   ...
   // }
    if ((come_cnt % 500) == 0) {
        ...
        VD_TRACE("%s, close audio stream ...", __func__);
    }
...
#define TIMES (1000)
    ...
    if (!h_CSpotter) {
        TRACE("CSpotter Init fail! err : %d\n", err);
        err = -3;
        goto fail;
    }
...
static void cmd_wait_handler(int state, void *param)
{
    ...
    VD_TRACE("%s, state=%d", __func__, state);
...
static void cpu_det_find_wakeup_handler(int state, void *param)
{
    ...
    cpu_wakeup_cnt++;
    VD_TRACE("%s, state=%d, cnt=%d", __func__, state, cpu_wakeup_cnt);
    //VD_TRACE("%s, calc sys freq=%d", __func__, hal_sys_timer_calc_cpu_freq(5,0));
...
//      while(1);
        ...
        if (sleep == HAL_SLEEP_STATUS_DEEP) {
            VD_TRACE("wake up from deep sleep");
        }
